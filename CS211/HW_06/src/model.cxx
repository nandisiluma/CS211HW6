...
{
    Position_set ps;
    if (find_move(pos) == nullptr){
        return ps; //if pos is occupied, return the empty position_set
    }
    for (auto dim: Board::all_directions()){
        ps |= find_flips(pos, dim); // add flippable positions to ps
    }
    ps[{pos}]; // add pos to the ps
    return ps;
}
